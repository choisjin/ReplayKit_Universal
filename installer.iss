...
; site-packages 제외: python\Lib\site-packages 를 번들하지 않음 → 인스톨러 빌드/용량 대폭 감소.
;   설치 후 setup.bat / ReplayKit.bat 가 requirements.txt 로 인터넷 설치 (lge.auto 는 로컬 .whl).
Source: "{#DistDir}\*"; DestDir: "{app}"; Excludes: "*.msi,VimbaX_Setup*,python-3.10.4-amd64.exe,Git-*.exe,vcredist_x64.exe,DltViewerSDK_21.1.3_ver,site-packages"; Flags: ignoreversion recursesubdirs createallsubdirs; Components: main
; VC++ Runtime (설치 후 삭제)
