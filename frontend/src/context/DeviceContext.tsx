...
export function DeviceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    } else if (dev.type === 'icas_agent' || dev.type === 'mib_agent' || dev.type === 'fpk_agent'
               || dev.type === 'gm_info_agent') {
      setScreenType('HU');
      ...
      if (typeof event.data === 'string') {
        ...
        try {
          ...
          if (msg.mode === 'h264') {
            ...
            console.warn('[mirror] backend error:', em);
            if (screenshotDeviceIdRef.current === deviceId) setStreamError(em);
