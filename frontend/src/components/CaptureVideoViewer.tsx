...
// Webcam.Capture 스텝 영상 뷰어 — 스텝 테스트 결과창 / 시나리오 결과 상세 공용.
//   영상: 결과 상세 녹화와 같은 VideoTransport 플레이어.
//   Frame으로 보기: 서버가 영상을 전 프레임 JPEG 로 분해 → 슬라이드 ↔ 그리드.
...
export default function CaptureVideoViewer({ videoPath, maxHeight = 460 }: Props) {
  ...
  const openFrames = async () => {
    ...
    let info = frames;
    if (!info) {
      const requested = videoPath;
      setLoading(true);
      setError('');
      try {
        const res = await resultsApi.captureFrames(requested);
        if (pathRef.current !== requested) return;
        info = res.data as FramesInfo;
        setFrames(info);
      } catch (e: any) {
        if (pathRef.current === requested) setError(e.response?.data?.detail || t('capture.framesFailed'));
      } finally {
        if (pathRef.current === requested) setLoading(false);
      }
    }
    if (info && info.count > 0) {
      setIndex(Math.min(info.count - 1, Math.max(0, Math.floor(cur * info.fps + 1e-6))));
    }
  };
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      const sec = Math.max(1, Math.round(frames.fps));
      let handled = true;
      switch (e.code) {
        case 'ArrowLeft': case 'KeyQ': setIndex(i => Math.max(0, i - 1)); break;
        case 'ArrowRight': case 'KeyE': setIndex(i => Math.min(frames.count - 1, i + 1)); break;
        case 'KeyA': setIndex(i => Math.max(0, i - sec)); break;
        case 'KeyD': setIndex(i => Math.min(frames.count - 1, i + sec)); break;
        case 'Home': setIndex(0); break;
        case 'End': setIndex(frames.count - 1); break;
      ...
      <>
        <div
          ...
          style={{
            ...
            maxHeight: maxHeight + 40, overflowY: 'auto', padding: 2,
          }}
        >
          {Array.from({ length: to - from }, (_, k) => from + k).map(i => (
            ...
            >
              ...
              />
              <div style={{
                fontSize: 10, textAlign: 'center', color: '#fff', background: 'rgba(0,0,0,0.6)',
                fontFamily: 'monospace', lineHeight: '15px',
              }}>
