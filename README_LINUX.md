...
- **자동 시작**: 아이콘 클릭 → GUI 가 뜨면 0.5초 후 서버 자동 시작
- **자동 브라우저 오픈**: 서버 ready 감지 (HTTP 응답) 후 기본 브라우저 자동 오픈 (최대 30초 대기)
- **▶/■** — 시작 / 종료 토글
- **↻** — 재시작
- **🌐** — 브라우저 열기 (수동 트리거)
...
| 변수 | 효과 |
| --- | --- |
| `REPLAYKIT_NO_AUTOSTART=1` | 앱 시작 시 서버 자동 시작 안 함 (사용자가 ▶ 직접) |
| `REPLAYKIT_NO_BROWSER=1` | 서버 ready 후 브라우저 자동 오픈 안 함 |
