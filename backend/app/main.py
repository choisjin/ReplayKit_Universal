...
async def websocket_screen_mirror(websocket: WebSocket):
    ...
    # WebOS(Connect Wide) 화면은 Android 캡처에 hole 로 뚫려 있어 scrcpy/screencap 으로는
    # 안 잡힌다(참조본도 Android 레이어의 hole 에 Linux 스트림을 합성한다). 그래서 미러는
    # iSAP 분기 그대로 두고, 서비스가 Linux VM linuxStream 프레임을 돌려준다.
    # 터치만 ADB(input) 로 나간다 — isap_agent_service 쪽 분기.
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if _now_err - _last_frame_error_log >= 10.0:
                    _last_frame_error_log = _now_err
                    logger.warning(
                        "Screen mirror frame failed: device=%s screen_type=%r — %s (%s)",
                        target_device_id, screen_type, e, type(e).__name__,
                    )
                try:
                    await websocket.send_json({
                        "type": "error",
