...
async def connect_device(req: ConnectRequest):
    ...
    if req.type == "adb":
        ...
        # ccIC / ccIC27 동일 QNX 클러스터 플랫폼(QNX SSH 포트10022). "ccic" 부분문자열로 매칭.
        # ccRC/Gen6/Connect 등 다른 모델은 제외. ccIC27은 "ccic27"로 더 좁게 구분.
        _model_l = (req.device_model or "").lower()
        _is_ccic = "ccic" in _model_l        # ccIC + ccIC27 공통
        _is_ccic27 = "ccic27" in _model_l    # ccIC27 전용
        # cluster display 인덱스: reference 기준 ccIC(기본)=display1(ccIC.py), ccIC27=display2.
        # 명시값 없거나 legacy 기본('1')일 때만 모델 기본 적용(구버전 프론트 '1' 하드코딩 보정).
        _cluster_display = str(ef.get("cluster_display") or "1")
        if _is_ccic and _cluster_display in ("", "1"):
            _cluster_display = "2" if _is_ccic27 else "1"
        # cluster SSH 비밀번호: ccIC/ccIC27 QNX(포트 10022)는 root/root. 비워두면 자동 root로 보정.
        _ssh_password = ef.get("ssh_password")
        if not _ssh_password and _is_ccic:
            _ssh_password = "root"
        # cluster 캡처 모드: ccIC/ccIC27은 TCP cluster(bits=1) 미지원이라 QNX SSH screenshot로 표시.
        # off=TCP, ssh=QNX SSH, chroma=배경+정보 합성. 미설정 ccIC/ccIC27 → ssh 자동.
        _composite_mode = str(ef.get("cluster_composite_mode") or "off")
        if _is_ccic and _composite_mode == "off":
            _composite_mode = "ssh"
        # cluster crop: ccIC/ccIC27 모두 QNX 캡처(2720x720)의 실제 cluster는 좌측 1920x720
        # (reference CLU_IMG_GET crop=0,0,1920,720). 우측 빈공간만 제거.
        _cluster_crop = str(ef.get("cluster_crop") or "")
        if _is_ccic and not _cluster_crop:
            _cluster_crop = "0,0,1920,720"
        try:
...
def _get_connected_mib(device_id: str):
    """mib_agent 디바이스 + 연결된 서비스 인스턴스 조회 (공통 검증)."""
    dev = dm.get_device(device_id)
    if not dev:
        raise HTTPException(status_code=404, detail=f"Device {device_id} not found")
    if dev.type != "mib_agent":
        raise HTTPException(status_code=400, detail=f"Device {device_id} is not a MIB agent")
    svc = dm.get_mib_service(device_id)
    if svc is None or not getattr(svc, "is_connected", False):
        raise HTTPException(
            status_code=409,
            detail=f"Device {device_id} is not connected. Connect first then retry.",
        )
    return dev, svc


@router.get("/mib/live-scene")
async def mib_live_scene(device_id: str):
    """레이어 칼리브레이션용 장면 조회 — 서피스 지오메트리 + 현재 오버라이드."""
    dev, svc = _get_connected_mib(device_id)
    import asyncio
    loop = asyncio.get_event_loop()
    try:
        scene = await loop.run_in_executor(None, svc.probe_live_scene)
    except Exception as e:
        logger.warning("MIB live-scene probe failed for %s: %s", device_id, e)
        raise HTTPException(status_code=500, detail=f"Scene probe failed: {e}")
    return {
        "device_id": device_id,
        "scene": scene,
        "overrides": dev.info.get("live_scene") or {},
    }


class MibLiveSceneRequest(BaseModel):
    device_id: str
    hmi_sid: str = ""          # "" = 자동
...
async def set_mib_live_scene(req: MibLiveSceneRequest):
    """레이어 칼리브레이션 저장 — 오버라이드 영구 저장 + 라이브 스트림 재시작."""
    ...
    loop = asyncio.get_event_loop()
    try:
        await loop.run_in_executor(None, svc.stop_live_stream)
        await loop.run_in_executor(None, svc.start_live_stream)
    except Exception as e:
        logger.warning("MIB live restart after scene calib failed: %s", e)
    dm._save_auxiliary_devices()
    return {"status": "ok", "device_id": req.device_id, "live_scene": ls}
